...
public class Sale {
    ...
    private void updateAgentExpenseTXRecords() {
        ...
        DB.runNonQuery(String.Format(@"
            ...
            SELECT A.AMOUNT * E.AMOUNT/100, A.AMOUNT * E.AMOUNT/100, 0, E.EXPENSETYPEID, A.USERID, A.ID, 1, ISNULL(ISNULL(PP.StartDate, S.SETTLEMENTDATE), S.ENTITLEMENTDATE), L.CREDITGLCODE, L.DEBITGLCODE, O.OFFICEMYOBCODE + U.INITIALSCODE, O.OFFICEMYOBCODE + U.INITIALSCODE, S.ADDRESS
            ...
            JOIN LIST O ON U.OFFICEID = O.ID
            LEFT JOIN PAYPERIOD PP ON S.PAYPERIODID = PP.ID
    ...
    public static void processBDImports(int BnDSalesID = -1, bool UserUpdate = false) {
        ...
        String szSQL = String.Format(@"
                    ...
                    SELECT {2} SL.ID AS BnDSALEID, *,
						(SELECT SUM(AMOUNT) FROM bdSALESVOUCHERDEDUCTION WHERE (REASON = 'External Conjunctional' OR REASON = 'Referral/Conjunctional' OR REASON like '%Conjunctional%') AND SALESVOUCHERID = SV.ID) AS CONJUNCTIONAL,
	                    SUBSTRING ( -- Calculate purchaser suburb(s) in a comma seperated list if required
		                    (SELECT ', ' + SUBURB FROM bdPROPERTY PUR_P
			                    JOIN bdCONTACT C ON C.RESIDENTIALPROPERTYID = PUR_P.ID
			                    JOIN bdCONTACTACTIVITY CA ON CA.CONTACTID = C.ID
			                    JOIN bdCONTACTACTIVITYTYPE CT ON CT.ID = CA.CONTACTACTIVITYTYPEID
                    ...
                    FROM SALE
                    WHERE BnDSALEID IN (
                        SELECT SL.ID FROM bdPROPERTY P
	                        JOIN bdSALESLISTING SL ON SL.PROPERTYID = P.ID
                    	    JOIN bdSALESVOUCHER SV ON SV.SALESLISTINGID = SL.ID
					    {1}
    ...
    /// <summary>
    /// Extracts appraisal information from the sales listing table
    /// </summary>
    public static void processAppraisals() {
        DB.runNonQuery(@"

        TRUNCATE TABLE bdAPPRAISALS;

        INSERT INTO bdAPPRAISALS(SALESLISTINGID, CONTACTID, APPRAISALCATEGORY, APPRAISAL_DATE, DAYCOUNT, STATUS)
	    SELECT DISTINCT SL.ID, ISNULL(LISTED.LISTED_CONTACT_ID, CANCEL_CONTACT_ID), ' ', LISTED.APPRAISAL_DATE, DATEDIFF(D, LISTED.APPRAISAL_DATE, SL.LISTEDDATE) AS DAYCOUNT, ''
        FROM bdSALESLISTING SL JOIN bdPROPERTY P ON P.ID = SL.PROPERTYID
	     LEFT JOIN bdCONSULTANT U1 ON U1.ID = SL.CONSULTANT1ID
	     LEFT JOIN bdCONSULTANT U2 ON U2.ID = SL.CONSULTANT2ID
	     LEFT JOIN bdCONSULTANT U3 ON U2.ID = SL.CONSULTANT3ID
	     LEFT JOIN (
		     SELECT SALESLISTINGID, MIN(CA.STARTDATE) AS CANCELLED_DATE, MIN(CA.CONTACTID) AS CANCEL_CONTACT_ID
		    FROM bdCONTACTACTIVITY CA JOIN bdCONTACTACTIVITYTYPE CAT ON CA.CONTACTACTIVITYTYPEID = CAT.ID AND CAT.NAME IN ('APPRAISAL - CANCELLED')
		     GROUP BY SALESLISTINGID
		     ) CANCEL ON CANCEL.SALESLISTINGID = SL.ID
	    LEFT JOIN (
		     SELECT SALESLISTINGID, MIN(CA.STARTDATE) AS APPRAISAL_DATE, MIN(CA.CONTACTID) AS LISTED_CONTACT_ID
		    FROM bdCONTACTACTIVITY CA JOIN bdCONTACTACTIVITYTYPE CAT ON CA.CONTACTACTIVITYTYPEID = CAT.ID AND CAT.NAME IN ('APPRAISAL - LISTED')
		     GROUP BY SALESLISTINGID
		     ) LISTED ON LISTED.SALESLISTINGID = SL.ID
	    LEFT JOIN bdAPPRAISALS A ON A.SALESLISTINGID = SL.ID
	    WHERE A.SALESLISTINGID IS NULL;

	     -- CATEGORY A
	     UPDATE bdAPPRAISALS SET APPRAISALCATEGORY = 'A'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM bdCONTACT C JOIN bdCONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN bdCONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 68);

	     -- CATEGORY B
	     UPDATE bdAPPRAISALS SET APPRAISALCATEGORY = 'B'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM bdCONTACT C JOIN bdCONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN bdCONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 70);

		 -- CATEGORY C
	     UPDATE bdAPPRAISALS SET APPRAISALCATEGORY = 'C'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM bdCONTACT C JOIN bdCONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN bdCONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 69);

	    --REMOVE ROWS THAT ARE INCOMPLETE
	    DELETE FROM bdAPPRAISALS WHERE CONTACTID IS NULL;
