...
public class Sale {
    ...
    public static void processAppraisals() {
        ...
        INSERT INTO APPRAISALS(SALESLISTINGID, CONTACTID, APPRAISALCATEGORY, APPRAISAL_DATE, DAYCOUNT, STATUS)
        ...
        FROM SALESLISTING SL JOIN PROPERTY P ON P.ID = SL.PROPERTYID
	     LEFT JOIN DB_USER U1 ON U1.ID = SL.CONSULTANT1ID
	     LEFT JOIN DB_USER U2 ON U2.ID = SL.CONSULTANT2ID
	     LEFT JOIN DB_USER U3 ON U2.ID = SL.CONSULTANT3ID
	     LEFT JOIN (
		     SELECT SALESLISTINGID, MIN(CA.STARTDATE) AS CANCELLED_DATE, MIN(CA.CONTACTID) AS CANCEL_CONTACT_ID
		    FROM CONTACTACTIVITY CA JOIN CONTACTACTIVITYTYPE CAT ON CA.CONTACTACTIVITYTYPEID = CAT.ID AND CAT.NAME IN ('APPRAISAL - CANCELLED')
	    ...
	    LEFT JOIN (
		    ...
		    FROM CONTACTACTIVITY CA JOIN CONTACTACTIVITYTYPE CAT ON CA.CONTACTACTIVITYTYPEID = CAT.ID AND CAT.NAME IN ('APPRAISAL - LISTED')
		     GROUP BY SALESLISTINGID
		     ) LISTED ON LISTED.SALESLISTINGID = SL.ID
	    LEFT JOIN APPRAISALS A ON A.SALESLISTINGID = SL.ID
	    WHERE A.SALESLISTINGID IS NULL;

	     -- CATEGORY A
	     UPDATE APPRAISALS SET APPRAISALCATEGORY = 'A'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM CONTACT C JOIN CONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN CONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 68);

	     -- CATEGORY B
	     UPDATE APPRAISALS SET APPRAISALCATEGORY = 'B'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM CONTACT C JOIN CONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN CONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 70);

		 -- CATEGORY C
	     UPDATE APPRAISALS SET APPRAISALCATEGORY = 'C'
	     WHERE CONTACTID IN
	    (SELECT C.ID
	    FROM CONTACT C JOIN CONTACTCATEGORY CC ON C.ID = CC.CONTACTID
	    JOIN CONTACTCATEGORYTYPE CCT ON CC.CONTACTCATEGORYTYPEID = CCT.ID AND CC.CONTACTCATEGORYTYPEID = 69);

	    --REMOVE ROWS THAT ARE INCOMPLETE
	    DELETE FROM APPRAISALS WHERE CONTACTID IS NULL;
