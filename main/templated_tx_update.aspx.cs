...
public partial class templated_tx_update : Root {
    ...
    private void bindData() {
        string szSQL = "Select * from USERTX WHERE ISTEMPLATE = 1 AND ISDELETED = 0ORDER BY USERID ";
        string szJSON = "var TX = '" + DataTableToJSON(DB.runDataSet(szSQL).Tables[0]) + "'";
        sbEndJS.Append(szJSON);
        szSQL = String.Format("Select ID, Name from LIST where LISTTYPEID = {0} ORDER BY NAME", (int)ListType.Expense);
