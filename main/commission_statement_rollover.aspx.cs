    ...
    public partial class commission_statement_rollover : Root {
        ...
        void createRecords() {
            DB.runNonQuery(String.Format(@"
				...
				SELECT 66, UPP.USERID, UPP.DISTRIBUTIONOFFUNDS, 0, DATEADD(month, 1, PP.ENDDATE), 'Carried forward balance', 1, 0, 1, UPP.DISTRIBUTIONOFFUNDS, 'FL-' + U.INITIALSCODE, 'FL-' + U.INITIALSCODE, 'FL-' + U.INITIALSCODE, 'FL-' + U.INITIALSCODE, 1
				FROM USERPAYPERIOD UPP JOIN DB_USER U ON UPP.USERID = U.ID JOIN PAYPERIOD PP ON PP.ID = UPP.PAYPERIODID
