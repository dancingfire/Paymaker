    ...
    public partial class off_the_top : Root {
        ...
        protected void bindData() {
            ...
            string szSQL = string.Format(@"
                ...
                S.SALEDATE, ISNULL((SELECT SE.CALCULATEDAMOUNT FROM SALEEXPENSE SE JOIN LIST L ON SE.EXPENSETYPEID = L.ID AND L.NAME = 'Gifts' WHERE SE.SALEID = S.ID), 0) as GIFTS,
                ISNULL((SELECT SE.CALCULATEDAMOUNT FROM SALEEXPENSE SE JOIN LIST L ON SE.EXPENSETYPEID = L.ID AND L.NAME = '6% Incentive' WHERE SE.SALEID = S.ID), 0) as INCENTIVE
