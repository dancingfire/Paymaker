...
namespace Paymaker {
    ...
    public partial class monthly_sales_by_agent : Root {
        ...
        protected void bindData() {
            string szFilter = String.Format(" S.SALEDATE BETWEEN '{0} 00:00:00' AND '{1} 23:59:59'", Utility.formatDate(dtStart), Utility.formatDate(dtEnd));
            ...
            string szUserIDFilter = Valid.getText("szUserID", G.CurrentUserRoleID != 1 ? G.User.UserID.ToString() : "", VT.List);
            if (!String.IsNullOrEmpty(szUserIDFilter)) {
                string szKPIFilter = "";
                if (Valid.getBoolean("blnApplyKPI", false))
                    szKPIFilter += " and USS.INCLUDEINKPI = 1 ";
                szFilter += String.Format(@"
                        AND S.ID IN (
                            SELECT SALEID
                            FROM SALESPLIT SS JOIN USERSALESPLIT USS ON SS.ID = USS.SALESPLITID {1}
                            WHERE USS.USERID IN ({0})) ", szUserIDFilter, szKPIFilter);
            }
